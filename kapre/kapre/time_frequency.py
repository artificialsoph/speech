...
class Spectrogram(Layer):
    ...
    def build(self, input_shape):
        ...
        dft_real_kernels, dft_imag_kernels = backend.get_stft_kernels(
            self.n_dft)
        self.dft_real_kernels = K.variable(
            dft_real_kernels, dtype=K.floatx(), name="real_kernels")
        self.dft_imag_kernels = K.variable(
            dft_imag_kernels, dtype=K.floatx(), name="imag_kernels")
...
class Melspectrogram(Spectrogram):
    ...
    def build(self, input_shape):
        ...
        mel_basis = backend.mel(self.sr, self.n_dft,
    ...
    def call(self, x):
        ...
        if self.image_data_format == 'channels_first':
            power_spectrogram = K.permute_dimensions(
                power_spectrogram, [0, 1, 3, 2])
        else:
            power_spectrogram = K.permute_dimensions(
                power_spectrogram, [0, 3, 2, 1])
